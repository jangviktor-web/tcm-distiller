...
[![Version](https://img.shields.io/badge/版本-V4.4d-blue)](CHANGELOG.md)
...
| **S10** | **诚实标注规范**（不编造，缺口显性化） |
| **S11** | **表达还原度验证**（20 题长答盲评，六维差距检查） |
| **S12** | **档案清理与索引健康维护**（删除后索引四步验证） |

### 🔬 实战验证的数据
- **倪海厦 258 方剂量勘误**：合格 233 / 修复 30+ 处 / 注册复查无回滚
...
### 环境要求
- IMA 平台（支持 `ima_skill_create` 注册）
- Python 3.8+（索引验证脚本）

### 安装
```bash
# 复制到技能目录并注册
cp -r tcm-distiller {平台skill目录}/
ima_skill_create -d {平台skill目录}/tcm-distiller
```

### 使用（两条流水线）
```
Pipeline A · 新蒸馏（人 → skill）
  Phase 0 入口分流 → Phase 1 十维并行调研 → Phase 2 框架合成+注册
...
├── CHANGELOG.md                # 完整版本历史（V1.0 → V4.4d）
├── references/
...
- 注册错误码速查表（411003/411006/200005）+ 平台快照覆盖防护
...
[![Star History Chart](https://api.star-history.com/svg?repos=jangviktor-web/tcm-distiller&type=Date)](https://star-history.com/#jangviktor-web/tcm-distiller&Date)
